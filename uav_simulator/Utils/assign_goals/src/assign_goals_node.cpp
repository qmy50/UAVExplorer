...
{
  // ROS_WARN("The drone num is %d",num_drone_);
  if(num_drone_ == 1){
      drones_.clear();
      Selected_t drone;
      drone.drone_id = 0;
      drone.p << 0.0f, 0.0f, 0.0f;
      drones_.push_back(drone);
      ROS_WARN("Only one drone ,no need to select !");
      Eigen::Vector3d user_goal(msg->pose.position.x, msg->pose.position.y, msg->pose.position.z);
      quadrotor_msgs::GoalSet goal_msg;
      goal_msg.drone_id = 0;
      goal_msg.goal[0] = user_goal(0);
      goal_msg.goal[1] = user_goal(1);
      goal_msg.goal[2] = user_goal(2);
...
int main(int argc, char **argv)
{
  ...
  nh.param("num_drone",num_drone_,0);
