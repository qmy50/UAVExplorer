...
// Internal mapping components — adapted for GridMap
...
class ObjectMap2D {
public:
  ObjectMap2D(GridMap::Ptr grid_map, ros::NodeHandle& nh);
  ~ObjectMap2D() = default;

  /// Ensure 2D map buffers are initialized (call before query methods)
...
private:
  ...
  // 2D grid coordinate helpers (adapted from SDFMap2D to GridMap's 2D layer)
  void posToIndex2D(const Eigen::Vector2d& pos, Eigen::Vector2i& idx);
  void indexToPos2D(const Eigen::Vector2i& idx, Eigen::Vector2d& pos);
  int toAddress2D(const Eigen::Vector2i& idx);
  int toAddress2D(const Eigen::Vector2d& pos);
  bool isInMap2D(const Eigen::Vector2i& idx);
  bool isInMap2D(const Eigen::Vector2d& pos);

  // Occupancy queries (delegated to GridMap's 2D layer)
  bool knownFree(const Eigen::Vector2i& idx);
  bool knownFree(const Eigen::Vector2d& pos);
  ...
  // System integration
  GridMap::Ptr grid_map_;  ///< Reference to the 3D GridMap (2D layer used for object mapping)

  // Cached 2D map parameters (from GridMap's 2D occupancy grid)
  double origin_x_, origin_y_, resolution_inv_;
...
inline bool ObjectMap2D::isConfidenceObject(const ObjectCluster& obj)
{
...
inline void ObjectMap2D::posToIndex2D(const Eigen::Vector2d& pos, Eigen::Vector2i& idx)
{
  idx(0) = int(floor((pos(0) - origin_x_) * resolution_inv_));
  idx(1) = int(floor((pos(1) - origin_y_) * resolution_inv_));
}

inline void ObjectMap2D::indexToPos2D(const Eigen::Vector2i& idx, Eigen::Vector2d& pos)
{
  pos(0) = (idx(0) + 0.5) * resolution_ + origin_x_;
  pos(1) = (idx(1) + 0.5) * resolution_ + origin_y_;
}

inline int ObjectMap2D::toAddress2D(const Eigen::Vector2i& idx)
{
  return idx(0) + idx(1) * width_;  // row-major, consistent with GridMap's 2D occupancy grid
}

inline int ObjectMap2D::toAddress2D(const Eigen::Vector2d& pos)
{
  Eigen::Vector2i idx;
  posToIndex2D(pos, idx);
  return toAddress2D(idx);
}

inline bool ObjectMap2D::isInMap2D(const Eigen::Vector2i& idx)
{
  return idx(0) >= 0 && idx(0) < width_ && idx(1) >= 0 && idx(1) < height_;
}

inline bool ObjectMap2D::isInMap2D(const Eigen::Vector2d& pos)
{
  Eigen::Vector2i idx;
  posToIndex2D(pos, idx);
...
// --- Occupancy queries (delegated to GridMap's 2D interface) ---

inline bool ObjectMap2D::knownFree(const Eigen::Vector2i& idx)
{
  Vector2d pos;
  indexToPos2D(idx, pos);
  return knownFree(pos);
}

inline bool ObjectMap2D::knownFree(const Eigen::Vector2d& pos)
{
  return grid_map_->is2DFree(pos(0), pos(1));
}

inline bool ObjectMap2D::checkSafety(const Eigen::Vector2i& idx)
{
  Vector2d pos;
  indexToPos2D(idx, pos);
  return checkSafety(pos);
}

inline bool ObjectMap2D::checkSafety(const Eigen::Vector2d& pos)
{
  // Unsafe if unknown or occupied in the 2D grid
  if (grid_map_->is2DUnknown(pos(0), pos(1)) || grid_map_->is2DOccupied(pos(0), pos(1)))
    return false;
  return true;
}
...
{
  if (isInMap2D(pos) && grid_map_->is2DOccupied(pos(0), pos(1)))
    return true;
  return false;
}
...
{
  return isObjectClustered(toAddress2D(idx));
}
...
inline bool ObjectMap2D::isNeighborUnknown(const Eigen::Vector2i& idx)
{
  ...
  for (auto nbr : nbrs) {
    Vector2d pos;
    indexToPos2D(nbr, pos);
    if (grid_map_->is2DUnknown(pos(0), pos(1)))
      return true;
  }
  return false;
}
...
inline int ObjectMap2D::getObjectGrid(const Eigen::Vector2i& id)
{
  ...
  return int(getObjectGrid(toAddress2D(id)));
}
...
inline void ObjectMap2D::publishObjectClouds()
{
  ...
  int skipped_no_label = 0, published = 0;
