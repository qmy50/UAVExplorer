...
def _lazy_box_convert():
    global _box_convert
    if _box_convert is None:
        try:
...
class ObjectDetections:
    ...
    def __init__(
        ...
        if fmt != "xyxy":
            self.boxes = _lazy_box_convert()(boxes=boxes, in_fmt=fmt, out_fmt="xyxy")
        else:
            self.boxes = boxes
        self.logits = logits
    ...
    def filter_by_conf(self, conf_thresh: float) -> None:
        ...
        """
        keep = _lazy_torch().ge(self.logits, conf_thresh)  # >=
        self._filter(keep)
    ...
    def filter_by_class(self, classes: List[str]) -> None:
        ...
        """
        keep = _lazy_torch().tensor(
            [p in classes for p in self.phrases], dtype=_lazy_torch().bool
        )
        self._filter(keep)

    def _filter(self, keep) -> None:
    ...
    def from_json(
        ...
        return cls(
            ...
            boxes=_lazy_torch().tensor(json_dict["boxes"]),
            logits=_lazy_torch().tensor(json_dict["logits"]),
...
def annotate(
    ...
    for box, prob, phrase in zip(boxes, logits, phrases):
        ...
        prob = prob.detach().cpu().numpy() if hasattr(prob, 'detach') else np.array([prob])
